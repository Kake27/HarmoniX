...
def detect_key_from_file(file_path: str, sr: int = 22050, window: float = 0.0, hop: float = 5.0,
                         use_hpss: bool = True, apply_tuning: bool = True, topk: int = 3):
    """
    Convenience wrapper that loads audio from file_path and runs detection.
    Returns the same tuple as detect_key_global or detect_key_windowed:
    - if window <= 0: (candidates, pcv, meta)
    - if window > 0: (segments_list, meta)
    """
    ...
    if window and window > 0:
        segments, meta = detect_key_windowed(y, sr_loaded, window_sec=window, hop_sec=hop,
                                            use_hpss=use_hpss, apply_tuning=apply_tuning, topk=topk)
        return segments, meta
    else:
        candidates, pcv, meta = detect_key_global(y, sr_loaded, use_hpss=use_hpss, apply_tuning=apply_tuning)
        return candidates[:topk], pcv, meta
